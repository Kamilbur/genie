 ...
 *
 * @brief Declaration of the Selector class template for routing data between
 * branches.
 *
 * This file contains the declaration of the `Selector` class template, which
 * serves as a multiplexer for directing data from one source to multiple
 * branches based on a defined operation. It combines the functionality of a
 * `Drain` and a `Source` to act as an intermediary in a data processing
 * pipeline, routing incoming data to specific outputs.
 *
 * @details The `Selector` class is used in scenarios where data needs to be
 * split and directed to different processing branches based on a given
 * condition or operation. It supports dynamic addition and configuration of
 * branches, allowing for flexible data flow management. The class template is
 * parameterized by the input type (`Tin`) and output type (`Tout`).
 */
 ...
 *
 * The `Selector` class is a generic template that allows data of type `Tin` to
 * be selectively routed to multiple branches. It inherits from `Drain<Tin>` to
 * consume data and `Source<Tout>` to produce data, acting as an intermediary in
 * a data pipeline. It uses a custom operation to determine which branch the
...
class Selector final : public Drain<Tin>, public Source<Tout> {
 ...
 public:
   ...
   *
   * This function adds a new input-output branch to the `Selector`. Each
   * branch consists of a `Drain<Tin>*` (input) and a `Source<Tout>*`
   * (output). The new branch will be appended to the list of branches managed
   * by the selector.
   *
   ...
   */
  void AddBranch(Drain<Tin>* entry, Source<Tout>* out);

  /**
   * @brief Sets a specific branch at the given index.
   *
   * This function updates the branch at the specified index with a new
   * input-output pair. If the index is out of bounds, it will resize the
   * branch list to accommodate the new entry.
   *
   * @param entry Pointer to the `Drain` representing the input branch.
   * @param out Pointer to the `Source` representing the output branch.
   * @param index The index at which to set the new branch.
   */
  void SetBranch(Drain<Tin>* entry, Source<Tout>* out, size_t index);
   ...
   * the configured operation. It uses the section identifier (`id`) to track
   * the data flow and ensure proper routing.
   *
